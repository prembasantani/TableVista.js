class tf_clusterize {
    constructor(tableBodyId, all_tr_tags, options = {}) {
        ...
        this.debounced_scroll_handler = this.debounce(
    ...
    start() {
        this.uniqueEventNamespace = `tf_clusterize_${
            this.tableBodyId
        }_${Date.now()}`;
        $(document).on(
            `scroll.${this.uniqueEventNamespace}`,
            this.debounced_scroll_handler
        );
    ...
    update(all_tr_tags, options) {
        ...
        let tbody_scrollTop = Math.floor(
            last_scrollTop - this.tbody_offset_top + this.offset_top
        );
        let percentage = Math.floor(
            (tbody_scrollTop / last_tbody_height_approx) * 100
        );
        ...
        $(this.tableBody)
            .children("tr:not(.tf_cluster_first_row, .tf_cluster_last_row)")
            .remove();
        ...
        if (tbody_scrollTop > start_scroll_limit) {
            let current_tbody_scrollTop =
                Math.floor((this.tbody_height_approx / 100) * percentage) -
                this.tbody_offset_top +
                this.offset_top;
    ...
    calculate_visible_rows_height() {
        let table_rows = $(this.tableBody).children(
            "tr:not(.tf_cluster_first_row, .tf_cluster_last_row)"
        );
    ...
    calculate_tbody_height() {
        ...
        this.tbody_height_approx =
            this.visible_rows_height_approx +
            this.rowHeight * (this.totalRows - this.visibleRows);
    }
    ...
    onScroll() {
        ...
        let tbody_scrollTop = Math.floor(
            scrollTop - this.tbody_offset_top + this.offset_top
        );
        ...
        if (is_scrolling_down) {
            ...
            if (
                tbody_scrollTop >=
                this.tbody_height_approx - this.rowHeight * this.bufferRows
            ) {
        ...
        let cluster_block_up = Math.floor(
            block_row_index / this.block_rows_for_rendering
        );
        ...
        if (cluster_block_up != 0) {
            ...
            if (is_scrolling_down) {
                start = Math.max(0, top_tr_row_index - this.bufferRows);
                    ...
                    start + number_of_rows_visible_in_dom,
                    end_index
                );
            } else {
            ...
            let is_height_matches =
                this.check_if_total_height_matches_tbody_height();

            if (
                start == first_dom_index &&
                end == last_dom_index &&
                is_height_matches
            ) {
                return;
            ...
            if (
                last_dom_index == end_index &&
                first_dom_index == start &&
                is_height_matches
            ) {
                return;
            }
        ...
        if (prepend) {
            for (let i = end; i >= start; i--) {
                if (!this.rowsInDom.includes(i)) {
                    ...
                    $(row).insertAfter(
                        "#" + this.tableBodyId + " tr.tf_cluster_first_row"
                    );
            ...
            for (let i = start; i <= end; i++) {
                if (!this.rowsInDom.includes(i)) {
                    row = this.all_tr_tags[i];
                    $(row).insertBefore(
                        "#" + this.tableBodyId + " tr.tf_cluster_last_row"
                    );
                    this.rowsInDom.push(i);
                    this.actual_row_index.push($(row).attr("row_index"));
                    row_count++;
    ...
    renderRows(start, end, prepend) {
        let tbody_exists_in_dom = document.getElementById(
            this.tableBody.getAttribute("id")
        );
    ...
    updateSpacerRow(recalculate) {
        ...
        if (first_visible_row_index == 0) {
            ...
            height_of_rows_hidden_from_bottom =
                this.tbody_height_approx - this.visible_rows_height_approx;
        } else if (last_visible_row_index == total_rows_in_table - 1) {
            // if last row is created in dom
            height_of_rows_hidden_from_top =
                this.tbody_height_approx - this.visible_rows_height_approx;
            this.calculate_visible_rows_height(); // recalculates the current visible row height "this.visible_rows_height_approx"
    ...
    check_if_total_height_matches_tbody_height() {
        let total_of_height =
            this.visible_rows_height_approx +
            this.first_row_height +
            this.last_row_height;
